...
6. release command 后监听 G1 actual position 和 motor current。opening-direction current 首次响应为
   detach event，G1 position 开始朝 520 移动作为二次确认；此刻读取 actual q/dq，经 FK/Jacobian
   与固定 grasp offset 得到 cube release position/velocity，再按重力传播。
...
camera 不进入 controller。third-view 只负责记录是否离手、旋转和接住；wrist camera 在本分支不存在。
...
D. 1× cube throw-only，下面放软垫，不闭合
E. 最多 2–3 次 dynamic regrasp
```

D 确认 actual detach、J5 空间和 cube 离手方向后才做 E。目标只是得到 2–3 个成功视频，不需要
...
- G1 commanded/actual position、motor current、三个 event 的 monotonic timestamp；
...
detach_state_source = g1_release_response

...
不要继续细扫 close time。下一步应从 stable reference 做 reference-level co-design：在 detach 后
先规划避开 right-inner-knuckle 的 clearance/brake path，再把下降段 intercept 写进整条受约束
joint reference；先重新获得 stable bilateral catch，然后再把 detach→apex 目标轴旋转从约 3°
提高到 ≥5°、整段提高到 ≥12°。所有候选继续受 1× 真机 joint envelope 和 80 ms lag 约束。

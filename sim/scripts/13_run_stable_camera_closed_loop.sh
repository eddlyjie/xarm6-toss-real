...
OUTPUT="${1:-$ROOT/outputs/stable_camera_closed_loop}"
CAMERA_SEED="${2:-20260816}"
mkdir -p "$OUTPUT"
...
OMNI_KIT_ACCEPT_EULA=Y ACCEPT_EULA=Y PRIVACY_CONSENT=Y \
  ...
  --observation-mode policy_cameras \
  --cube-size-m 0.035 \
  --cube-mass-kg 0.025 \
...
echo "stable_camera_closed_loop_output=$OUTPUT"
echo "camera_seed=$CAMERA_SEED"
echo "summary=$OUTPUT/summary.json"
echo "probe_j=$OUTPUT/probe_j.json"
echo "camera_measurements=$OUTPUT/camera_measurements.json"
